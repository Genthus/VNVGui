<template>
    <div class="flex flex-row gap-4 p-4">
        <dialogueView
            class="basis-3/4 bg-white shadow-inner rounded-xl"
            @save="saveScene"
        ...
        />
        <dialogueOptions
            class="grow bg-white shadow-inner rounded-xl"
...
function loadDialogues(sceneId) {
    fetch(
        "http://localhost:5000/getScene?projectId=" +
...
function loadResources() {
    fetch(
        "http://localhost:5000/getResourceList?projectId=" +
            route.params.projectId
    )
        .then((response) => {
            if (!response.ok) {
...
async function saveScene() {
    await fetch(
        "http://localhost:5000/saveScene?projectId=" +
            route.params.projectId +
            "&sceneId=" +

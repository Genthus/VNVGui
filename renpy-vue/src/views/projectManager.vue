<template>
    <div class="w-full">
        <div
            class="container shadow-md mt-3 rounded-lg p-6 m-auto bg-purple-400"
        >
            ...
            >
                <ul class="" :key="project.id" v-for="project in projects">
                    ...
                    >
                        ...
                        >
                            <span class="text-lg text-right font-bold">{{
                                project.name
                            }}</span>
                        </div>
                        <div
...
async function createProject() {
    const result = await Validate(newProjData.value, rules);
    if (result.length > 0) {
    ...
    const response = await fetch(
        "http://localhost:5000/createProject?name=" + newProjData.value.name
    );
    if (!response.ok) {
...
async function deleteProject(name) {
    const response = await fetch(
        "http://localhost:5000/deleteProject?name=" + name
    );
    if (!response.ok) {
